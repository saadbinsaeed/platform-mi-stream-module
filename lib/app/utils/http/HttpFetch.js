...
var _config = _interopRequireDefault(require("app/config/config"));
...
const extractMessage = data => {
  ...
  let message = String(data.message || data || '');
...
const handleResponse = response => response.text().then(text => {
  ...
  if (!response.ok) {
    ...
    error.statusText = response.statusMessage; //FIXME: remove text.indexOf(...) >= 0 when the API are correctly returning 401

    if (error.status === 401 || text.indexOf('Cannot call sendError() after the response has been committed') >= 0) {
      if (_config.default.IN_IFRAME) {
...
const fetchData = (url, requestType, requestBody, headers) => {
  return _affectliSso.default.updateToken(5).then(() => {
    const options = {
      // $FlowFixMe
      method: requestType,
      headers: new Headers({
    ...
    return fetch(`${_config.default.API_ENTRY_POINT}/${url}`, options);
  }).then(response => {
...
class HttpFetch {
   ...
   * @throws Error
   */
  static getResource(uri, headers) {
  ...
  static deleteResource(uri, headers) {
    if (!uri) throw new Error('URI is required parameter.');
  ...
  static postResource(uri, body, headers) {
    if (!uri) throw new Error('URI is required parameter.');
  ...
  static putResource(uri, body, headers) {
    if (!uri) throw new Error('URI is required parameter.');
    if (!body) throw new Error('`body` is required parameter.');
  ...
  static uploadFile(uri, file, fieldName = 'attachment', headers) {
    const formData = new FormData();
    formData.append(fieldName, file, file.name);
    return fetch(`${_config.default.API_ENTRY_POINT}/${uri}`, {
      method: 'post',
      headers: new Headers({
        Authorization: _affectliSso.default.getBearerToken(),
        ...headers
  ...
  static postForm(uri, operations, file, headers) {
